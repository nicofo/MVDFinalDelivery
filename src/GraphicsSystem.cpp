...
void GraphicsSystem::init(int window_width, int window_height, std::string assets_folder) {
	...
	extra_gbuffer_.initGbuffer(window_width, window_height);
	frame_.initGbuffer(window_width, window_height);
...
int GraphicsSystem::renderEntityToTexture(int entity_id) {

	
	renderEntityToTexture(entity_id, lm::vec4(0,0,0, 1));
	/* ENVIRONMENT */
	// We want the skybox in this case
	renderEnvironment_();
	/* VIEW FRAMES */
	return frame_.color_textures[0];
}

int GraphicsSystem::renderEntityToTexture(int entity_id, lm::vec4 background_color) {
	...
	/* SHADOW PASS FOR ALL LIGHTS */
	auto& mesh_component = ECS.getComponentFromEntity<Mesh>(entity_id);

	/* SCREEN BUFFER */
	// TODO CONFIG frame buffer to save it in a texture
	bindAndClearScreen_(background_color);
	frame_.width = viewport_width_;
	frame_.height = viewport_height_;
	frame_.bindAndClear(background_color);
	resetShaderAndMaterial_();
	...
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	glEnable(GL_BLEND);
		checkShaderAndMaterial_(mesh_component);
		renderMeshComponent_(mesh_component);
	if (ECS.hasComponent<SkinnedMesh>(entity_id)) {
		auto& skinnedmesh_component = ECS.getComponentFromEntity<SkinnedMesh>(entity_id);
		checkShaderAndMaterial_(skinnedmesh_component);
		renderSkinnedMeshComponent_(skinnedmesh_component);
	}
	return frame_.color_textures[0];
}
...
void GraphicsSystem::bindAndClearScreen_(lm::vec4 cc) {
	glViewport(0, 0, viewport_width_, viewport_height_);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
